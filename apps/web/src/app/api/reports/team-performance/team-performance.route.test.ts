...
/** GET /api/reports/team-performance — gated `viewReports`. */
...
vi.mock("@destaworks/application/reports/team-reports.service", () => ({
  teamReportsService: { teamPerformance: h.teamPerformance },
}));

import { GET } from "./route";

const getReq = () => new Request("http://localhost/api/reports/team-performance");

beforeEach(() => {
  h.session = null;
  h.teamPerformance.mockReset();
});
...
describe("GET /api/reports/team-performance", () => {
  it("401 when signed out", async () => {
    ...
    expect(h.teamPerformance).not.toHaveBeenCalled();
  });
  ...
  it("200 for Owner, delegates to the service", async () => {
    ...
    const result = { rows: [] };
    h.teamPerformance.mockResolvedValue(result);
    const res = await GET(getReq(), undefined);
