...
/** GET /api/reports/trends — gated `viewReports`. */
...
vi.mock("@destaworks/application/reports/trends.report", () => ({
  trendsReport: { trends: h.trends },
}));

import { GET } from "./route";

const getReq = () => new Request("http://localhost/api/reports/trends");
...
describe("GET /api/reports/trends", () => {
  it("401 when signed out", async () => {
    ...
    expect(h.trends).not.toHaveBeenCalled();
  });
  ...
  it("200 for Owner, delegates to the service", async () => {
    ...
    const result = { metrics: [], anomalies: [], funnel: [] };
    h.trends.mockResolvedValue(result);
    const res = await GET(getReq(), undefined);
