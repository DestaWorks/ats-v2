...
export function useUrlFilters({ resetPage = false }: { resetPage?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- debounce keyed on `search` alone; urlSearch/setParam change per render and would cancel the timer before it ever fires.
  }, [search]);
