...
import { PROSPECT_STATUSES } from "@destaworks/domain/constants";
import type {
  ProspectListDTO,
  ProspectListItemDTO,
} from "@destaworks/contracts/validation/prospect";
import { messageForFailure } from "@/lib/api/client";
import { Button, buttonClasses } from "@destaworks/ui/button";
...
import { Table } from "@destaworks/ui/table";
import { pageHrefFor } from "@destaworks/domain/pagination";
...
export function ProspectsInventory({
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- keyed on idsKey, the serialized id list; `live` is derived from it, so listing it would re-run on every identical render.
  }, [idsKey]);
