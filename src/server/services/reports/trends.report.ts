import "server-only";
import { dateKey, daysBefore } from "@/lib/daily";
import type {
  TrendsAnomalyDTO,
  TrendsDTO,
...
import { dailyRepository, type InstantWindow } from "@/server/repositories/daily.repository";
...
export const trendsReport = {
  async trends(): Promise<TrendsDTO> {
    ...
    const funnel: TrendsFunnelStageDTO[] = funnelKeys.map((key, i) => {
      const m = metrics.find((x) => x.key === key)!;
      const prevKey = i > 0 ? funnelKeys[i - 1] : null;
      const prevMetric = prevKey ? metrics.find((x) => x.key === prevKey)! : null;
      return {
        label: m.label,
        curr: m.thisWeek,
        prev: m.lastWeek,
