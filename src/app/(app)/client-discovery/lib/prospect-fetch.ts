...
export function getProspectDetail(
  id: string,
  signal?: AbortSignal,
): Promise<ApiResult<{ prospect: ProspectDetailDTO }>> {
...
export function deleteProspect(id: string): Promise<ApiResult<{ ok: true; id: string }>> {
  return deleteJson(`/api/prospects/${id}`);
}

/** Restore a soft-deleted prospect. Returns the fresh detail. */
export function postRestoreProspect(
  id: string,
): Promise<ApiResult<{ prospect: ProspectDetailDTO }>> {
...
export function postEnrichApollo(id: string): Promise<ApiResult<{ prospect: ProspectDetailDTO }>> {
  return postJson(`/api/prospects/${id}/enrich`, {});
}

/** Hunter.io fallback when Apollo has no result. Returns the fresh detail. */
export function postEnrichHunter(id: string): Promise<ApiResult<{ prospect: ProspectDetailDTO }>> {
