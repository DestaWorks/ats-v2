...
export function parseMentionTokens(body: string): string[] {
  ...
  for (const [, token] of body.matchAll(MENTION_TOKEN_RE)) {
    if (token) seen.add(token.toLowerCase());
  }
  return [...seen];
}

/** The first whitespace-separated word of a name (`""` when the name is blank). */
...
export function mentionToken(user: MentionTarget, all: readonly MentionTarget[]): string {
  const first = firstWord(user.name);
  const collides = all.some(
    (u) => u.id !== user.id && firstWord(u.name).toLowerCase() === first.toLowerCase(),
  );
...
export function resolveMentions(body: string, users: readonly MentionTarget[]): MentionTarget[] {
  ...
  for (const token of parseMentionTokens(body)) {
    ...
    const [user, ...ambiguous] = matches;
    if (!user || ambiguous.length > 0) continue; // unknown or ambiguous → no notification
