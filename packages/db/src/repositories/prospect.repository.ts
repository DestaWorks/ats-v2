...
export const prospectRepository = {
  create(data: Prisma.ProspectUncheckedCreateInput, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.create({ data });
  },

  findById(id: string, opts?: { includeDeleted?: boolean }, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.findFirst({
  ...
  list(filters: ProspectListFilters = {}, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.findMany({
  ...
  count(filters: ProspectListFilters = {}, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.count({ where: buildProspectWhere(filters) });
  },

  /** Patch a prospect — status / owner / notes. */
  update(id: string, data: Prisma.ProspectUncheckedUpdateInput, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.update({ where: { id }, data });
  },

  softDelete(id: string, actorId: string, tx?: Prisma.TransactionClient) {
    return db(tx).prospect.update({
      where: { id },
  ...
  createMany(rows: Prisma.ProspectCreateManyInput[], tx?: Prisma.TransactionClient) {
    return db(tx).prospect.createMany({ data: rows, skipDuplicates: true });
  },
};
