 ...
 *
 * SECURITY (PII/PHI): resume text and the extracted structured output hold the most sensitive
...
/** ~60k-char cap on the resume text sent to the model (mirrors the legacy limit). */
 ...
 */
const COMMON_PROMPT = [
  "You are a resume parser for DestaHealth, a US mental-health recruiting agency.",
  "Extract data from the supplied resume into the required structured fields.",
  "Rules:",
  ...
  '(3) For "snapshot", you may synthesize a 3–4 sentence sales-grade paragraph, but only restate what is actually in the resume — do not invent achievements. Quantify where the candidate quantified.',
  "(4) For experience bullets, you may tighten phrasing but never add achievements that are not in the resume. Lead with verbs. Keep numbers from the resume.",
...
/**
 * Extract a resume with the configured LLM provider. Gated on `aiEnabled` (the provider's key
