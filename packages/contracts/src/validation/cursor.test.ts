...
describe("cursor codec", () => {
  ...
  it("throws when the row is missing the field its sort order walks", () => {
    expect(() => encodeCursor({ id: "c1" }, "createdAt_desc")).toThrow(/createdAt/);
    expect(() => encodeCursor({ id: "c1" }, "at_desc")).toThrow(/at/);
    expect(() => encodeCursor({ id: "c1" }, "name_asc")).toThrow(/name/);
  });

  it("encodes an empty name (falsy but present) rather than throwing", () => {
    const encoded = encodeCursor({ name: "", id: "c1" }, "name_asc");
    expect(decodeCursor(encoded, "name_asc")).toEqual({ kind: "name", value: "", id: "c1" });
  });
