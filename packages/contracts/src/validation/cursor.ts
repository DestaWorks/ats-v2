...
/**
 * The sort value a row contributes to its cursor. A row missing the field its sort order walks
 * cannot produce a usable cursor (it would encode `null` and decode back to a 400), so this is a
 * caller bug — it throws rather than emitting a cursor that fails one request later.
 */
function cursorValue(row: CursorSource, kind: CursorKind): string {
  if (kind === "name") {
    if (row.name === undefined) throw new TypeError('cursor row is missing "name"');
    return row.name;
  }
  const when = kind === "at" ? row.at : row.createdAt;
  if (when === undefined) throw new TypeError(`cursor row is missing "${kind}"`);
...
export function encodeCursor(row: CursorSource, orderBy: AnyOrderBy): string {
  const value = cursorValue(row, orderByKind(orderBy));
