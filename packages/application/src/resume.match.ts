import type { ResumeData, ResumeMatch } from "@destaworks/contracts/validation/resume";
...
/** Read one cell of a Levenshtein DP row. Rows are filled left-to-right before they are read, so
 *  an out-of-range read is a bug in the loop bounds, not a missing value. */
function cell(row: number[], index: number): number {
  const value = row[index];
  if (value === undefined) throw new RangeError(`levenshtein: no DP cell at ${index}`);
  return value;
}
...
function levenshtein(a: string, b: string): number {
  ...
  for (let i = 1; i <= m; i++) {
    ...
    for (let j = 1; j <= n; j++) {
      ...
      const value = Math.min(left + 1, cell(prev, j) + 1, cell(prev, j - 1) + cost);
      curr[j] = value;
      left = value;
    }
    [prev, curr] = [curr, prev];
