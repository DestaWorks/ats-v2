...
import { templatePerformanceRepository } from "@destaworks/db/repositories/template-performance.repository";
    ...
    for (const [templateId, rows_] of byTemplate) {
      ...
      const days = leadRows
        .flatMap((a) =>
          a.respondedAt == null
            ? []
            : [Math.floor((a.respondedAt.getTime() - a.at.getTime()) / MS_PER_DAY)],
        )
