...
export async function createUser(
  ...
  await post(request, "/admin/users", { name, email, roleId, password });
}

/**
 * Resolve a role NAME to the acting workspace's own role row id.
 *
 * Roles are tenant-owned rows, so `POST /admin/users` names a `roleId` rather than a value from a
 * fixed enum — a workspace may rename its tiers or invent new ones, which makes a name an unstable
 * identifier. Specs still say "Associate" because that is what they mean; this turns it into the
 * id that workspace actually holds.
 */
async function roleIdFor(request: APIRequestContext, name: string): Promise<string> {
  ...
  if (!response.ok()) {
    throw new Error(`GET /tenants/roles failed: ${response.status()} ${await response.text()}`);
  }
  const { roles } = (await response.json()) as { roles: { id: string; name: string }[] };
  const match = roles.find((role) => role.name === name);
  if (match === undefined) {
    throw new Error(`No role named "${name}" here (have: ${roles.map((r) => r.name).join(", ")})`);
  }
  return match.id;
}
